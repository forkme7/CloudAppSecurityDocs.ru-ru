...
-   Cloud App Security теперь обеспечивает более глубокую интеграцию с Azure Information Protection, позволяя защищать файлы в G Suite. Эта функция (пока в режиме общедоступной предварительной версии) позволяет проверять и классифицировать файлы в G Suite, а также автоматически применять метки Azure Information Protection для защиты. Дополнительные сведения см. в разделе [Интеграция Azure Information Protection](azip-integration.md).

-   Теперь Cloud Discovery поддерживает i-FILTER.

-   Таблица агентов SIEM теперь включает более подробные сведения, что упрощает управление.
...
- Начато развертывание компонента прокси-сервера Microsoft Cloud App Security. Прокси-сервер Microsoft Cloud App Security предоставляет средства, с помощью которых можно в реальном времени отслеживать и контролировать доступ к облачной среде, а также выполняемые в ней действия. Пример.
