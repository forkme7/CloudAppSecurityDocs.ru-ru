---
title: "Интеграция SIEM | Microsoft Docs"
...
2. Извлеките JAR-файл из ZIP-файла и запустите его на сервере.
 После запуска файла выполните следующую команду:
      ...
      java -jar siemagent-0.87.20-signed.jar [--logsDirectory название_каталога] [--proxy адрес[:порт]] --token токен
> [!NOTE]
> Имя файла зависит от версии агента SIEM.

Здесь "название_каталога" — путь к каталогу, который вы хотите использовать для локальных журналов отладки агента.
...
1. Убедитесь, что агент SIEM на портале Cloud App Security не находится в состоянии Disconnected (Отключен) и что оповещения для этого агента отсутствуют. 
...
Если в будущем потребуется изменить агент SIEM, щелкните многоточие в конце строки агента SIEM в таблице и выберите **Edit** (Изменить). При измерении агента SIEM повторно запускать JAR-файл не нужно, он будет обновлен автоматически.
