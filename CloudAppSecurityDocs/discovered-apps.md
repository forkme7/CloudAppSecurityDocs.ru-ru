...
Функция Cloud Discovery позволяет анализировать использование облака в организации, определяя конкретные используемые экземпляры в обнаруженных поддоменах.

Например, вы можете оценивать различия между сайтами SharePoint.

Это поддерживается только в брандмауэрах и прокси-серверах, которые содержат данные целевых URL-адресов. Список поддерживаемых устройств см. в описании [поддерживаемых брандмауэров и прокси-серверов](set-up-cloud-discovery.md#supported-firewalls-and-proxies).
