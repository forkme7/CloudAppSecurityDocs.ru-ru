...
Cloud App Security позволяет автоматически применять метки классификации Azure Information Protection с защитой или без нее к файлам как действие управления политики файлов. Вы также можете изучать файлы с помощью фильтрации по примененной метке классификации на портале Cloud App Security. Это расширяет возможности для просмотра конфиденциальных данных и управления ими в облаке. Процесс интеграции Azure Information Protection с Cloud App Security прост: нужно установить всего один флажок. 
...
- Поддержка файлов PDF и файлов изображений будет доступна в будущих версиях. 
...
5. Вы также можете удалять метки классификации с помощью параметра **Удаление метки классификации**. 
...
> [!NOTE]
> Функция автоматического применения меток Azure Information Protection с использованием политики файлов предоставляет мощные возможности. Чтобы защитить пользователей от ошибочного применения меток к большому числу файлов, в качестве меры предосторожности настроено ежедневное ограничение в 100 действий **применения меток** для каждого приложения и каждого клиента. По достижении ежедневного ограничения действие применения метки временно приостанавливается. Оно автоматически включается на следующий день (после 12:00 UTC). Чтобы увеличить ограничение для вашего клиента, [обратитесь в службу поддержки Cloud App Security](mailto:cascoresupport@microsoft.com).
